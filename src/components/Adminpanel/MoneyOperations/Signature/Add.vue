    ...
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">{{$t('signature.add')}}</h5>
          <button
            type="button"
            class="close"
            data-dismiss="modal"
    ...
    add() {
      ...
      this.$appAxios
        ...
        .then((res) => {
          ...
          this.form.status = '1';
          this.$store.state.Opr.signatures.unshift(res.data.data);
          this.$notify({
            type: "success",
            title: "Signatures successfully added.",
          });
        })
        .catch((err) => {
